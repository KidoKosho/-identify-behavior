...
def export_tracking_records(records, output_path, scale=1.0, homography=None, columns=None):
    ...
    if output_path:
        fieldnames = list(records[0].keys()) if records else list(resolved_columns)
        if records and "x_center" in records[0] and "y_center" in records[0]:
            if "x_center_m" not in fieldnames:
                fieldnames.append("x_center_m")
            if "y_center_m" not in fieldnames:
                fieldnames.append("y_center_m")
        with open(output_path, "w", newline="", encoding="utf-8") as f:
